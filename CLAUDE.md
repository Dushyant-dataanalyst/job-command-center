...
- Kite Connect (`kite_fallback.py`): daily manual token via `kite_auth_refresh.yml` workflow_dispatch; session in `kite_session.json` (expires daily). Two roles now: (a) quote-price *fallback* when yfinance fails; (b) real portfolio reads (Phase 1 done) — `get_holdings/get_positions/get_margins/get_trades/get_orders`. `kite_portfolio_refresh.py` writes `kite_portfolio.json` (real holdings/margin/reconciliation vs manual tracking) each run, failing closed+graceful (`session_live:false`) when token stale; dashboard "Real Portfolio" panel in My Positions. **No orders** (SEBI static IP). `kite_portfolio.json` is a *public* static feed (user accepted holdings exposure). Phase 2 (not built): Kite historical candles as primary signal source, yfinance fallback, via a `market_data.get_ohlcv()` choke point + instrument-token cache.
